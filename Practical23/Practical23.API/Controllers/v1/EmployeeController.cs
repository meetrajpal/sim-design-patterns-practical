namespace Practical23.API.Controllers.v1;
...
public class EmployeeController(IEmployeeService _employeeService) : Controller
{
    ...
    [HttpGet("{employeeId}")]
    public async Task<IActionResult> OvertimePayment(string employeeId, decimal hours)
    {
        var result = await _employeeService.GetEmployeeOvertimePaymentAsync(employeeId, hours);
        return Ok(result);
    }
}
