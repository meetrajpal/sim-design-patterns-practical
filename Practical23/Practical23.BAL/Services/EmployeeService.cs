...
public class EmployeeService(IUnitOfWork unitOfWork, IEmployeeMapper employeeMapper, IFileLogger logger, IAbstractFactoryStore _abstractFactoryStore) : IEmployeeService
{
    ...
    public async Task<ApiResponse<decimal>> GetEmployeeOvertimePaymentAsync(string id, decimal hours)
    {
        logger.Log($"Employee overtime payment calculation started.");

        if (!Guid.TryParse(id, out Guid empId))
        {
            logger.LogError($"Invalid format for Employee Id {id}, it should be valid Guid.", null);
            throw new KeyNotFoundException($"Invalid format for Employee Id {id}, it should be valid Guid.");
        }
        ...
        if (employee == null)
        {
            logger.LogError($"Employee not found with given ID {id}.", null);
            throw new KeyNotFoundException($"Employee not found with given ID {id}.");
        }

        var res = _abstractFactoryStore.GetAbstractFactory(employee.Department.DepartmentName).GetFactory(employee.Department.DepartmentName).Create().CalculateOvertimePayment(hours);

        logger.Log("Employee overtime payment successfully calculated.");

        return ApiResponse<decimal>.Success(res, "Overtime Payment successfully calculated.");
    }
}